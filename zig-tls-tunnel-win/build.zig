...
pub fn build(b: *std.Build) void {
    ...
    exe.addObjectFile(b.path(ssl_lib));
    exe.addObjectFile(b.path(crypto_lib));
    ...
    if (optimize != .Debug) {
        ech_exe.root_module.strip = true;
    }
    ...
    if (optimize != .Debug) {
        profile_exe.root_module.strip = true;
    }
